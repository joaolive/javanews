...
public final class Article extends Post {
	...
	private Article(UUID id, Body body, UUID authorId, Instant createdAt, Instant updatedAt, Title title, Slug slug,
			Set<Tag> tags) {
	...
	public static Article create(Title title, Slug slug, Body body, UUID authorId, Set<Tag> tags) {
		Instant now = Instant.now();
		return new Article(UUID.randomUUID(), body, authorId, now, now, title, slug, tags);
	}

	public static Article reconstitute(UUID id, String title, String slug, String body, UUID authorId, Instant createdAt,
		Instant updatedAt, Set<Tag> tags) {
		return new Article(id, Body.restore(body), authorId, createdAt, updatedAt, Title.restore(title), Slug.restore(slug), tags);
	}
